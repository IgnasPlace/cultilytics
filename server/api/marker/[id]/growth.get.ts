...
export default defineEventHandler(async (event) => {
  try {
    ...
    const records = await db.query.plantGrowthRecords.findMany({
      where: eq(tables.plantGrowthRecords.markerId, markerId),
    ...
    if (error instanceof H3Error) {
      ...
      throw createError({
        statusCode: 500,
        statusMessage: "Failed to fetch growth records",
      });
    }
  }
});
